...
begin
	...
	M_AXI_AWADDR	<= std_logic_vector(unsigned(C_M_CDMA_BASE_ADDR) + unsigned(axi_awaddr));-- std_logic_vector (unsigned(targetSlave) + unsigned(axi_awaddr));
	--AXI 4 write data
	...
	--Read Address (AR)
	M_AXI_ARADDR	<= std_logic_vector(unsigned(targetSlave) + unsigned(axi_araddr));
		  ...
		  begin                                                                             
		    if (rising_edge (M_AXI_ACLK)) then                                              
		      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                
		        ...
		        -- Signals a new write address/ write data is                               
		        -- available by user logic                                 
				if writeReset = '1' and axi_bready = '1' then
					axi_wdata <= controlWord or x"00000004";
				else
					axi_wdata <= x"0000000F";                                   
				end if;                  
	      ...
	      if (M_AXI_ARESETN = '0' ) then                                                                
	        ...
	        case (mst_exec_state) is                                                                    
	          ...
	          when INIT_READ =>                                                                         
				...
				-- 	if reads_done = '1' then
				-- 		if writeFlag = '1' then       
				-- 		mst_exec_state <= INIT_WRITE;--INIT_COMPARE; 
				-- 		end if;
				-- 		if writeFlag = '0'then
				-- 			mst_exec_state <= IDLE;--INIT_COMPARE;  
				-- 		end if;
				-- 	end if;
				-- end if;                                                                                                
	            -- 	if reads_done = '0' then
	            --   		mst_exec_state  <= INIT_READ;                                                         
	            --   		if (axi_arvalid = '0' and M_AXI_RVALID = '0' and last_read = '0' and                  
