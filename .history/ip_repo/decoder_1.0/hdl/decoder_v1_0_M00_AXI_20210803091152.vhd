...
--	signal decoderState : decodestate;
	...
	signal controlWord : std_logic_vector(C_M_AXI_ADDR_WIDTH-1 downto 0);
...
begin
	...
	M_AXI_AWADDR	<= x"42000000";  --std_logic_vector(unsigned(C_M_CDMA_BASE_ADDR) + unsigned(axi_awaddr));-- std_logic_vector (unsigned(targetSlave) + unsigned(axi_awaddr));
	--AXI 4 write data
	...
	--Read Address (AR)
	M_AXI_ARADDR	<= x"42000000"; --std_logic_vector(unsigned(targetSlave) + unsigned(axi_araddr)); 
	...
	begin
		if rising_edge (M_AXI_ACLK) then
			if insFinishPulse = '0' AND insFinishPulseF = '0' then --insFinishPulseF
				if writeFlag = '1' then
					if insFinishPulseFF = '1' then--if M_AXI_RVALID = '1' then 
						rdDmaFlag <= '1';
					end if;		
					if M_AXI_RVALID = '1' then
						cdmaWriteFlag <= '1';
					...
					--axi_araddr <= x"00000004";--std_logic_vector(to_unsigned(CDMASR, C_M_AXI_DATA_WIDTH));
					--status <= M_AXI_RDATA;--cdmardResult;
					--read status

					-- cdmardAddr <= C_M_CDMA_BASE_ADDR + CDMASR;
					-- controlWord <= M_AXI_RDATA;--cdmardResult;
				end if;
				-- if writeFlag = '0' then
				-- 	if insFinishPulseFF = '1' then--if M_AXI_RVALID = '1' then 
				-- 		rdDmaFlag <= '0';
			...
			-- if writeReset = '1' then
			-- 	--cdmaWrite(C_M_CDMA_BASE_ADDR, CDMASR, (controlWord AND x"00000004"));--RESET AXI CDMA
			-- 	if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                
	    	--     	axi_awaddr <= (others => '0');                                              
			-- 	elsif (M_AXI_AWREADY = '1' and axi_awvalid = '1') then   
			-- 		axi_araddr <= C_M_CDMA_BASE_ADDR + std_logic_vector(to_unsigned(CDMASR, C_M_AXI_DATA_WIDTH));
			-- 		axi_wdata <= (controlWord AND x"00000004"); --RESET AXI CDMA
			-- 		end if;
			-- 	end if;
			-- if resetFinish = '1' then
			-- 	--axi_araddr <= CDMACR + (controlWord AND x"fffffffb"); --RESET AXI CDMA
			-- end if;
			-- if initInter = '1' then
			-- 	--axi_araddr <= CDMACR + (controlWord AND x"00007000"); --RESET AXI CDMA
			-- end if;
			--cdmaRead(cdmaBase : std_logic_vector(C_M_AXI_DATA_WIDTH-1 downto 0), cdmaOffset : std_logic_vector(C_M_AXI_DATA_WIDTH-1 downto 0))
		--end if;
	end process;

	READ_CDMA: process( M_AXI_ACLK )
	begin
		if rising_edge (M_AXI_ACLK) then
			 ...
			 --axi_araddr <= std_logic_vector(to_unsigned(CDMACR, C_M_AXI_DATA_WIDTH));
				...
				-- end if;
			 	if resetFinish = '1' then
					...
					if axi_awvalid = '1' then --M_AXI_WREADY = '1' and axi_wvalid = '1'
						resetFinish <= '1';
					end if;
				else
					if axi_awvalid = '1' then --M_AXI_WREADY = '1' and axi_wvalid = '1'
	...
	--keep track with number of read transaction issued/initiated
	  ...
	  begin                                                                            
	    if (rising_edge (M_AXI_ACLK)) then                                             
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1' or rdDmaPulse = '1') then                                               
	        read_index <= (others => '0');                                           
	      else                                                                         
	  ...
	  begin                                                                            
	    if (rising_edge (M_AXI_ACLK)) then                                             
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1' or rdDmaPulse = '1') then                                               
	        ...
	        if (start_single_read = '1' OR insFinishPulse = '1' OR cdmaWriteFinishPulseF = '1') then                                          
	...
	-- Read Addresses                                                                      
	    process(M_AXI_ACLK)                                                                
	   	  begin                                                                         
	   	    if (rising_edge (M_AXI_ACLK)) then                                          
	   	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then --or ( rdDmaFlag = '1' and writeInit = '0' )) then                                            
	   	        axi_araddr <= (others => '0');                                          
	        ...
	        case (mst_exec_state) is                                                                    
	          ...
	          when INIT_WRITE =>                                                                        
	            ...
	            if (writes_done = '1') then                                                                              
					...
					-- 	mst_exec_state  <= INIT_READ;    
					-- else 
					mst_exec_state <= INIT_READ;--INIT_READ;      
					-- end if;                                                                          
	            end if;    
	            ...
	            -- read controller       
						if (reads_done = '1') then                                                              
	            		  ...
	            		  if ((axi_arvalid = '0' and M_AXI_RVALID = '0' and last_read = '0' and                  
	            		    start_single_read = '0' and read_issued = '0' and insFinishPulse = '0' )or (cdmaWriteFinishPulseF = '1')) then 
...
--	*/                                                                                               
	...
	-- */                                                                                               
	  ...
	  begin                                                                                             
	    if (rising_edge (M_AXI_ACLK)) then                                                              
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1' or insFinishPulseFF = '1') then                                                                
	        reads_done <= '0';                                                                          
	      else                                                                                          
