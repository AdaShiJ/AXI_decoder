	...
	begin
		if rising_edge (M_AXI_ACLK) then
			if insFlag = '1' then
				...
				if (M_AXI_RVALID = '1' and counter = 4 and BTTFlag  = '0') then
					insFinish <= '1';
					end if;	
			end if;
	end if;
		  ...
		  begin                                                                             
		    if (rising_edge (M_AXI_ACLK)) then                                              
		      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then  --or (cdmaWriteStartPulse = '1')) then                                                
				...
				elsif (M_AXI_WREADY = '1' and axi_wvalid = '1' ) then--and writeCombine = '0') then  
					if  BTTFlag = '1' then
						axi_wdata <= x"00000008";
					elsif DAFlag = '1' and BTTFlag = '0' then
						axi_wdata <= x"c0000000";
					elsif DAFlag = '0' and SAFlag = '1' then
						axi_wdata <=  x"44A00000";--axi_wdata <=  x"40000000";
					elsif resetFinish = '1' and initInter = '0' then
						axi_wdata <= controlWord or x"00007000";
					elsif writeReset = '1' and resetFinish = '0' then
						axi_wdata <= controlWord and x"fffffffb";
				...
				if cdmaWriteFlag = '1' then
					if BTTFlag = '1' then
						...
						axi_wdata <= x"c0000000";  
					elsif  DAFlag = '0' and initInter = '1' then--SAFlag = '1' then
						axi_wdata <=  x"44A00000"; --axi_wdata <=  x"40000000";  
					elsif  resetFinish = '1' and initInter = '0' then
						axi_wdata <= controlWord or x"00007000";
					elsif  writeReset = '1' and resetFinish = '0' then
	      ...
	      if (M_AXI_ARESETN = '0' ) then                                                                
	        ...
	        case (mst_exec_state) is                                                                    
	          ...
	          when IDLE =>                                                                      
				...
				-- end if;                                                                                                                              
	            -- end if;               
				if ( init_txn_pulse = '1') then --and (not(insFinish = '0' and cdmaWriteFlag = '0' and BTTFlag = '1'))) then    
				  mst_exec_state <= INIT_READ;  
	              --mst_exec_state  <= INIT_WRITE;                                                        
	              ERROR <= '0';
	              compare_done <= '0';
