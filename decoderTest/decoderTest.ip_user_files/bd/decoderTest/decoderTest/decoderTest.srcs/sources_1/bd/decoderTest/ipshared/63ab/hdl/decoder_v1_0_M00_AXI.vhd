...
begin
	...
	M_AXI_AWADDR	<= std_logic_vector (unsigned(targetSlave) + unsigned(axi_awaddr));
	--AXI 4 write data
	...
	begin
		if rising_edge (M_AXI_ACLK) then
			if (insFinishPulseF = '0' and init_txn_pulse = '0' and M_AXI_ARESETN = '1' and axi_arvalid = '0') then
				if writeFlag = '1' then
					rdDmaFlag <= '1';
	   	  ...
	   	  begin                                                                         
	   	    if (rising_edge (M_AXI_ACLK)) then                                          
	   	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then --or ( rdDmaFlag = '1' and writeInit = '0' )) then                                            
	   	        axi_araddr <= (others => '0');                                          
	   	      elsif (M_AXI_ARREADY = '1' and axi_arvalid = '1' and rdDmaFlag = '0') then                    
	   	        -- Signals a new write address/ write data is                           
	   	        -- available by user logic                                              
	   	        ...
	   	        -- Signals a new write address/ write data is                           
	   	        -- available by user logic                                              
	   	        axi_araddr <= std_logic_vector(to_unsigned(CDMASR, C_M_AXI_DATA_WIDTH));--x"00000000"; 
	        ...
	        case (mst_exec_state) is                                                                    
	          ...
	          when INIT_WRITE =>                                                                        
	            ...
	            if (writes_done = '1') then                                                                              
	            	if rdDmaFlag = '1' then
						mst_exec_state  <= INIT_READ;    
					else mst_exec_state <= IDLE;--INIT_READ;      
					end if;                                                                          
	            end if;    
				if (writes_done = '0')then
					mst_exec_state  <= INIT_WRITE;                                                                                                                                       
